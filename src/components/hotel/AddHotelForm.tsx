...
import { Eye, Loader2, PencilLine, Plus, Terminal, Trash, XIcon } from "lucide-react";
...
const formSchema = z.object({
  ...
  locationDescription: z.string().min(3, {
    message: "Location must be atleast 3 character long"
  }),
...
const AddHotelForm = ({ hotel }: Props) => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    setIsLoading(true);
    if (hotel) {
      axios.patch(`/api/hotel/${hotel.id}`, values).then((res: any) => {
        toast({
          variant: 'success',
  ...
  const handleDeleteHotel = async (hotel: HotelWithRooms) => {
    ...
    try {
      ...
      await axios.delete(`/ap/hotel/${hotel.id}`)
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className=" flex flex-row space-x-3 rounded-md items-end border p-4">
                        ...
                        <FormLabel>Free Parking Space</FormLabel>
                      </FormItem>
                    )}
                  />
                  <FormField
            ...
            <div className="flex-1 flex flex-col gap-6">
              ...
              {hotel && !hotel.rooms.length && <Alert className=" bg-indigo-600 text-white">
                ...
                <AlertDescription>
                  ...
                  <div className="">Please add some rooms to complete your hotel setup!</div>
                </AlertDescription>
              </Alert>}
              ...
              <div className="flex justify-between gap-2 flex-wrap">
                ...
                {hotel && <Button type="button" variant='outline' onClick={() => router.push(`/hotel-details/${hotel.id}`)}><Eye className=" mr-2 h-4 w-4" /> View</Button>}
              ...
              {hotel && !!hotel.rooms.length &&  <div>
                  <Separator />
                  <h3 className=" text-lg font-semibold my4
                  ">Hotel Rooms</h3>
